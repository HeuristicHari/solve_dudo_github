...
def strip_one_off(x):
    if x == 0:
        return 0 #TODO: maybe remove this
...
def main():
    ...
    d1_yhistory_to_constraintindex[(0, 0)] = 0 #root=1

    constraint_index = 0
    for d1 in priv_p1:
        constraint_index += 1
        d1_yhistory_to_constraintindex[(d1, 0)] = constraint_index
    ...
    for (d2, yhistory) in d2_hist_to_yindex.keys():
        ...
        for d1 in priv_p1:
            constraint_index = d1_hist_to_xindex[(d1, xhistory)] + len(priv_p1)
